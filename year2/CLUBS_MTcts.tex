...
\date{\vspace*{2cm} -- v0.2.3 --\\March 2018}
...
\begin{document}
	...
	Table~\ref{tab:cts-per-db} and table~\ref{tab:its-per-db} give an overview about the available descriptive term data available for each database segment.
		...
		\begin{tabular}{lrrrr}
			...
			\midrule
			MEDLINE           & 90,4\%/96\% & 90,4\%/96\% & 90,4\%/96\% & -/-\\
			ERIC       &    53,2\%/98,9\% &    -/- &    -/- & -/-\\
			ISOC   &    -/- &    -/- &    -/- & -/-\\
			NARCIS &       -/- &       -/- &       -/- & -/-\\
			NORART &       -/94,2\% &       -/- &       -/- & -/-\\
...
\end{table}
	...
	\footnote{PubPsych record set as of 4th August 2017 with     1,037,536  entries.}  using the fields {\tt CTDL}, {\tt CTEL}, {\tt CTFL} and {\tt CTSL}. Table~\ref{tab:ct} shows the statistics per language. Notice that there are no {\tt CTSL} (Spanish CTs) but we did not retrieve any result for {\tt CTSH} either, the other field with information related to controlled terms.
